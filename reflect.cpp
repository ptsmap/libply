...
struct MyStruct {
    ...
    char f;
    // 使用宏注册结构体成员
    REFLECTABLE(MEMBER_INFO(MyStruct, a, int),MEMBER_INFO(MyStruct, g, unsigned int), MEMBER_INFO(MyStruct, b, float), MEMBER_INFO(MyStruct, c, double),
    MEMBER_INFO(MyStruct, d, short),MEMBER_INFO(MyStruct, e, unsigned short),MEMBER_INFO(MyStruct, f, char)
    )
};
